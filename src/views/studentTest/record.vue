...
<style>
	...
	<div>
		...
		<div class="testN">
			<div><span class="login"></span>{{mytest.testInfo.testName}}<span class="totalS">得分：{{totalScore}}分</span></div>
		</div>
		<div class="testC" style="height: 80px;">
			...
			<span class="testSpan"><img src="../../assets/img/test2.png" />考试时间：{{mytest.testInfo.testTime}}分钟</span>
			<span class="testSpan"><img src="../../assets/img/test3.png" />总分：{{mytest.questionInfo.examinationPaper.examinationPaperScore}}分</span>
			...
			<span class="testSpan"><img src="../../assets/img/test5.png" />考试截止时间：{{ mytest.testInfo.endTime | moment('YYYY-MM-DD HH:mm')}}</span>
...
<script>
import { saveMA,getMA,submitT,getRecord } from '@/api/student'
...
export default {
	...
	methods: { 
	getR(){
		let tid = this.$route.query.testId;
		let sid = this.$route.query.studentId;
		let cid = this.$route.query.classId;
		this.totalScore = this.$route.query.testScore;
		let userType = getUserType();
		getRecord(userType,tid,sid,cid).then((response) => {
    		this.mytest=response.data;
//  		for(var i=0;i<response.data.questionInfo.questionTypes[0].typeQuestions.length;i++){
//  			this.score1+=response.data.questionInfo.questionTypes[0].typeQuestions[i].myScore;
//  		}
//  		for(let i=0;i<response.data.questionInfo.questionTypes[1].typeQuestions.length;i++){
//  			this.score2+=response.data.questionInfo.questionTypes[1].typeQuestions[i].myScore;
//  		}
//  		for(let i=0;i<response.data.questionInfo.questionTypes[2].typeQuestions.length;i++){
//  			this.score3+=response.data.questionInfo.questionTypes[2].typeQuestions[i].myScore;
//  		}
//  		for(let i=0;i<response.data.questionInfo.questionTypes[3].typeQuestions.length;i++){
//  			this.score4+=response.data.questionInfo.questionTypes[3].typeQuestions[i].myScore;
//  		}
//  		this.totalScore=this.score1+this.score2+this.score3+this.score4;
         })
        .catch((response)=> {
