<template>
	<div>
		...
		<div class="testN">
			<div><span class="login"></span>{{mytest.testInfo.testName}}</div>
		</div>
		<div class="testC">
			...
			<div class="testT">
				<div class="testCon">
					...
					<el-button type="primary" class="testJ" @click="submitTest">提交答卷</el-button>
				</div>
				<div>
...
export default {
	data(){
		return{
			mytest:JSON.parse(localStorage.getItem("mytests")),
			test:[],
    ...
    countdown(){
      ...
      setTimeout(function () {
        ...
        if(JSON.parse(localStorage.getItem("test")).length==0){
              	data.push({
              		studentId:that.custId,
	        		testId:that.mytest.testInfo.testId,
	        		examinationPaperId:that.mytest.questionInfo.examinationPaper.examinationPaperId
              	})
              }else{
          	 for(let i in JSON.parse(localStorage.getItem("test"))){
        	data.push({
        		studentId:that.custId,
        		testId:that.mytest.testInfo.testId,
        		examinationPaperId:that.mytest.questionInfo.examinationPaper.examinationPaperId,
        		questionId:JSON.parse(localStorage.getItem("test"))[i].questionId,
        		myAnswer:JSON.parse(localStorage.getItem("test"))[i].myAnswer,
        	})
        	}
          }       
...
//      });
        ...
        const client = new OSS.Wrapper({
            region: _this.region,
            secure: true, 
            accessKeyId: _this.mytest.credentials.accessKeyId,
            accessKeySecret:_this.mytest.credentials.accessKeySecret,
            stsToken: _this.mytest.credentials.securityToken,
            bucket: _this.bucket
        })
         _this.percentage = 0;
              // 随机命名
    	...
    	this.$confirm('提交之后将不能进行考试，是否继续?', '提示', { type: 'warning' })
              ...
              if(JSON.parse(localStorage.getItem("test")).length==0){
              	data.push({
	        		...
	        		testId:this.mytest.testInfo.testId,
	        		examinationPaperId:this.mytest.questionInfo.examinationPaper.examinationPaperId
              	})
              }else{
              	for(let i in JSON.parse(localStorage.getItem("test"))){
              	if(JSON.parse(localStorage.getItem("test"))[i].myAnswer==null || JSON.parse(localStorage.getItem("pTest"))[i].myAnswer==""){
              		a++;	
              	}else{
              		a+=0;
              	}
              ...
              if(a==JSON.parse(localStorage.getItem("test")).length){
