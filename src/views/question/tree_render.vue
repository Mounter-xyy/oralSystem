<template>
	<span class="tree-expand">
		...
		<span v-show="!DATA.isEdit">
			<svg-icon icon-class="file" /><span @click="showQuestion(STORE,DATA,NODE)">&nbsp;{{DATA.label}}&nbsp;</span>
		</span>
		<span class="tree-btn" v-show="!DATA.isEdit && teacherLevel=='1'">
			<i class="el-icon-plus" @click.stop="nodeAdd(STORE,DATA,NODE)"></i>
			<i class="el-icon-edit" @click.stop="nodeEdit(STORE,DATA,NODE)"></i>
			<i class="el-icon-delete" @click.stop="nodeDel(STORE,DATA,NODE)" v-if="DATA.id!=0"></i>
		</span>
	</span>
</template>

<script>
	import { editTree } from '@/api/tree'
	export default{
		name: 'treeExpand',
		data(){
			return {
		...
		methods: {
			...
			nodeEditPass(s,d,n){//编辑完成
				editTree(d.id,d.label).then(response => {
       })
				d.isEdit = false;
			},
			nodeEditFocus(){
