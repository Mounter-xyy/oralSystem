                ...
                <div class="col-md-12">
                    ...
                    <ul class="btn-edit fl">
                        <li class="input-serach">
                            <el-input placeholder="请输入学生姓名" v-model="keywords1" style="width: 150px;"></el-input>
                            <el-date-picker
							  class="input-class"
...
export default {
	...
	query(){
		this.filter.studentName = this.keywords1;
		this.getPL();
	},
	peaceDetail(user){
		const {href}=this.$router.resolve({
  			path:'/peaceDetail',
			name:'PeaceDetail',
  			query: {unitPaperId: user.unitPaperId,studentName: user.studentName,studentId: user.studentId,testScore: user.score}
  			})
